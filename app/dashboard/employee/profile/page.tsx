 ...
 * Defines the types for label, value, and editability
 */
...
export default function EmployeeProfile() {
  // Fetching the current session from NextAuth client-side hook
      ...
      <div className="bg-white rounded-[48px] p-10 border shadow-sm">
        <div className="flex flex-col md:flex-row gap-10 items-center">
          ...
          <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
            {/* Optional chaining used to safely access session data */}
...
/**
 * Reusable InfoBox component for profile fields
 * Handles both read-only and editable states
 */
function InfoBox({ label, value, isEditable = false }: InfoBoxProps) {
  return (
    <div className="space-y-2">
      ...
      <input 
        ...
        defaultValue={value} 
        readOnly={!isEditable}
        className={`w-full p-4 rounded-2xl border text-sm font-bold italic transition-all ${
          isEditable 
            ? 'bg-white border-slate-200 focus:border-blue-500 outline-none shadow-inner' 
            : 'bg-slate-50 border-transparent cursor-not-allowed text-slate-500'
        }`}
      />
    </div>
  );
